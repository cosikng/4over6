...
void accepts(void *args)
{
	...
	while (1)
	{
		tmp = accept(sock, (struct sockaddr *)&client_addr, &len);
		pthread_mutex_lock(&mutex);
		close(client);
		client = tmp;
		rst = 1;
...
int main(int argc, char **argv)
{
	...
	err = bind(sock, (struct sockaddr *)&local_addr, sizeof(struct sockaddr_in6));
	if (err == -1)
	{ /*判断错误*/
		perror("bind error");
		return (1);
	...
	pthread_create(&pt, NULL, (void *)accepts, NULL);
